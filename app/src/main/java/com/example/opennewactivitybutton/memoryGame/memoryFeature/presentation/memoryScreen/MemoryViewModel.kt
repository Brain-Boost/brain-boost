...
import com.example.opennewactivitybutton.memoryGame.memoryFeature.presentation.util.NumericalValues
...
class MemoryViewModel: ViewModel() {
    ...
    private fun increaseMatches() {
        if (_state.value.pairCount < NumericalValues.max_pairs) {
            _state.value = _state.value.copy(
    ...
    private fun decreaseMatches() {
        if (_state.value.pairCount > NumericalValues.min_pairs) {
            _state.value = _state.value.copy(
                pairCount = _state.value.pairCount - 1
            )
        }
